...
mock.onGet('/api/scrumboard-app/boards').reply(() => {
    const response = _.map(scrumboardDB.boards, (board) => _.pick(board, ['id', 'name', 'uri', 'color']));
    return [200, response];
});
...
mock.onGet('/api/scrumboard-app/board').reply((config) => {
    const {routineId} = config.params;
    const response = _.find(scrumboardDB.boards, {id: routineId});
    if ( response )
    {
        return [200, response];
    }
