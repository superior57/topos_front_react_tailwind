...
function Routine(props)
{
    ...
    const routine = useSelector(({routinesApp}) => routinesApp.routine.data);
    const file = useSelector(({routinesApp}) => routinesApp.routine.file);
    const [isOrganizer, setIsOrganizer] = useState(false);
    const classes = useStyles();

    useEffect(() => {
        // dispatch(Actions.getRoutine(props.match.params));
    }, [dispatch, props.match.params]);

    useEffect((a) => {if(!_.isEqual(form, routine)){setForm(routine)}}, [routine, props.params]);

    useEffect((a) => {
        if(routine && routine.organizerMembers != null){
            const organizerMember = routine.organizerMembers.filter(u => u.id === user.id)
            if (organizerMember !== null) {setIsOrganizer(true)}
        }
    }, [routine, props.params]);

    useEffect(() => {
        if(form && file && form.avatar !== file.link){
        {setForm(_.set({...form}, 'avatar',file.link))}
        {setForm(_.set({...routine}, 'avatar',file.link))}
    }},[file]) 
    ...
    {
        if(form){
        return (!_.isEqual(routine, form));
        }
    }

    const handleSubmit = (form) =>
    {
        // dispatch(Actions.saveRoutine(form));
    }
                    ...
                    <div className="flex flex-1 w-full items-center justify-between">
                        ...
                        <div className="flex flex-1 flex-col items-center sm:items-start">
                            ...
                            <FuseAnimate animation="transition.slideRightIn" delay={300}>
                                ...
                                role="button" to="/apps/routines" color="inherit">
                    ...
                    <div className="p-16 sm:p-24 w-full">
                        <div className="d-flex">
                            ...
                            <div className="flex flex-row">
                                ...
                                {isOrganizer && 
                                <Link 
                                    to={{pathname:`/user-manager/${routine.id}`, 
                                    state:{
                                        basicMembers: routine.basicMembers, 
                                        organizerMembers: routine.organizerMembers,
...
export default withReducer('routinesApp', reducer)(Routine);
